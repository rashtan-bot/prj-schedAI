...
class Schedule:
    def __init__(self):
        self._data = data  # Assuming 'data' is defined elsewhere in your code
        self._classes = []
        self._numberOfConflicts = 0
        self._fitness = -1
        self._classNumb = 0
        self._isFitnessChanged = True
        self._assigned_rooms = {}  # Track room assignments for sections
    ...
    def initialize(self):
        ...
        available_rooms = list(self._data.get_rooms())  # Get all rooms as a list

        for section in sections:
            dept = section.department
            n = section.num_class_in_week
            meeting_times = list(self._data.get_meetingTimes())  # Convert to list for indexing

            # Assign a consistent room for the section
            if section.section_id not in self._assigned_rooms:
                if available_rooms:
                    assigned_room = available_rooms.pop(0)  # Assign the first available room
                    self._assigned_rooms[section.section_id] = assigned_room
                else:
                    print(f"No rooms available for section {section.section_id}.")
                    continue

            assigned_room = self._assigned_rooms[section.section_id]
            ...
            if n <= len(meeting_times):
                ...
                for course in courses:
                    for i in range(n // len(courses)):
                        ...
                        # Assign consistent room for this section
                        newClass.set_room(assigned_room)

                        # Set instructor
                        if crs_inst:
                            newClass.set_instructor(crs_inst[rnd.randrange(0, len(crs_inst))])
                        else:
                            print(f"No instructors available for course: {course}.")
    ...
    def calculate_fitness(self):
        ...
        for i in range(len(classes)):
            ...
            # Check for time conflicts (room and instructor)
...
def context_manager(schedule):
    ...
    for cls in classes:
        context.append({
            ...
            "meeting_time": f'{cls.meeting_time.day} at {cls.meeting_time.time}'
        })
    return context
...
def timetable(request):
    ...
    if population.get_schedules()[0].get_fitness() != 1.0:
        print("Optimal schedule not found within the generation limit.")

    # Sort classes by meeting day and time
    sorted_schedule = sorted(schedule, key=lambda cls: (cls.meeting_time.day, cls.meeting_time.time))
